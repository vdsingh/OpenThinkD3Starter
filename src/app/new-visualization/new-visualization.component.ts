...
export class NewVisualizationComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    let parentObj = this.formatData();
    // console.log("Parent:");
    const root = pack(parentObj);
    ...
    leaf.append("text")
      ...
      .selectAll("tspan")
      .data(d => d.data._id.split(/(?=[A-Z][a-z])|\s+/g))
      .join("tspan")
      .attr("x", 0)
  ...
  formatData(){
    ...
    for(let i = 0; i < this.relations.length; i++){
        let parent = postsMap.get(this.relations[i].post1);
        let child = postsMap.get(this.relations[i].post2);
    ...
    return roots.length == 0 ? roots[0] : {value: 1, children: roots};
  }
